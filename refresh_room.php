...
while($row = mysqli_fetch_array($rem_rooms)) {
		...
		for ($x = $starttime; $x <= $endtime-1; $x++) {
            ...
            $end=($x+1).':00';
   			...
   			mysqli_query($con, "INSERT INTO rooms (room_name, room_from, room_to, type, day)VALUES('$name', '$start', '$end', '$type', '$R_day')");
    ...
    $i=0;
        ...
        for ($x = $starttime; $x <= $endtime-1; $x++) {
            ...
            mysqli_query($con, "INSERT INTO rooms (room_name, room_from, room_to, type, day)VALUES('$name', '$start', '$end', '$type', '$day')");
...
while($row = mysqli_fetch_array($del_rooms)) {
    ...
    // mysqli_query($con, "DELETE FROM rooms WHERE name= 'Teacher1' AND time_from='8:00' AND time_to='9:00' AND day='Monday'");
    mysqli_query($con, "DELETE FROM rooms WHERE room_name='$name' AND room_from='$time_from' AND room_to='$time_to' AND day='$day'");
